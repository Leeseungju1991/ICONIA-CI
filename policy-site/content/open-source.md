# 오픈소스 라이선스 고지 (Final Review-Ready)

> **본 문서는 법무 검토를 위한 final 수준 본문이다.** 라이선스 자동 생성 script (`6. CI/scripts/generate-oss-notice.ps1`) 가 출시 직전 의존성 inventory 를 갱신한다. 본문 표현 검토 후 그대로 시행 가능.
...
3. GPL/AGPL/Commercial-only 라이선스 → 사내 법무 검토 후 도입 결정
4. MIT/BSD/Apache-2.0/ISC 등 → 자동 승인

## 9. 시행

- 본 정책 작성일: 2026-05-28
- **시행 예정일**: 출시일 (사내 법무 검토 + OSS notice 자동 생성 완료 후 확정)
- 최종 OSS notice list 는 출시 빌드 직전 자동 생성 → 앱 내 포함

---

> **법무 검토 메모**: 본 정책은 「저작권법」 의 오픈소스 라이선스 의무 + LGPL §6 재링크 의무를 충족하는 표현으로 작성됐다. 본문 자체는 시행 가능. 의존성 list 는 자동 생성 결과로 갱신.
