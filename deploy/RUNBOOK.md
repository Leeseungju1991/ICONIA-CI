...
## 0-Z. PoC 잔재 — 출시 전 필수 조치

> 아래 항목은 PoC(개념 증명) 한정 조치이며, 출시 전 반드시 강화해야 한다.

### PoC Operator 자격증명 (admin/admin)

| 항목 | 현재 값 (PoC) | 출시 전 조치 |
|---|---|---|
| 이메일 | `admin@soomkorea.com` | 유지 (도메인 정합) |
| 비밀번호 | `admin` (argon2id 해시 저장) | 강력한 비밀번호로 교체 필수 |
| role | `superadmin` | 유지 |
| MFA | `mfa_required=false` | `mfa_required=true` + TOTP 등록 필수 |

**admin web 입력 규칙:** 로그인 폼에 `admin` 입력 →
`DEFAULT_EMAIL_DOMAIN=soomkorea.com` 으로 자동 변환 → `admin@soomkorea.com` 전송.

**출시 전 필수 조치:**

1. ADMIN 웹에서 비밀번호 변경 (또는 SSM 으로 새 argon2id 해시 직접 업데이트).
2. TOTP 앱(Google Authenticator 등) 으로 TOTP 등록 — `/api/v1/operator/totp/enroll` → `/verify`.
3. `/etc/iconia.server.env` 에서 `OPERATOR_MFA_ENFORCE_ROLES=` 를 `all` 또는 `superadmin` 으로 복귀.
4. `admin@iconia.io` 는 이미 `admin@soomkorea.com` 으로 rename 완료 (2026-06-24).
   구 email 로는 로그인 불가 — 잔존 기록 없음.

**비밀번호 서비스 정책 참고:**
- `operatorService.checkPasswordPolicy`: 최소 12자 + 영문+숫자+특수문자 (현재 PoC 비번 정책 위반).
- 해시 생성 시 정책 우회(직접 argon2.hash 호출)로 주입함 — 운영 상태에서 password reset 시
  정책을 통과하는 비밀번호만 저장 가능.
