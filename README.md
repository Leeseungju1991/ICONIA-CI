...
- **GitHub Actions / CodePipeline / CodeBuild**: 1~5는 GitHub 에 없으므로 클라우드 CI 는 적용 불가.
  로컬 빌드가 곧 빌드 단계.
- **RDS**: 결정에 따라 제거됨 (2026-05-18). 필요 시 EC2 위에 PostgreSQL 자체 설치 또는 EFS 위
  SQLite/LMDB. 향후 트래픽 증가 시 `terraform/` 에 rds.tf 를 재추가.
